...
class AccountTest(unittest.TestCase):
    def setUp(self):
        api_key = '615f9450-87ab-406a-8274-e500c9ad4c20'
        api_secret_key = 'AD393118420B206C54AAA7DF06E0EEFB'
        passphrase = 'DCV0419dcv!'
    ...
    #     print(self.AccountAPI.get_account_bills())
    #
...
# ------------------------------------------------------------------------
    ...
    # def test_get_positions_history(self):
    #     try:
    #         positions_history = self.AccountAPI.get_positions_history()
            
    #         self.assertIsInstance(positions_history, dict, "Positions history should be a dictionary")
    #         self.assertIn('data', positions_history, "Response should contain 'data' key")
    #         self.assertIsInstance(positions_history['data'], list, "Positions data should be a list")
            
    #         print("\n===== Positions History =====")
    #         print(f"Total positions: {len(positions_history['data'])}")
            
    #         if positions_history['data']:
    #             print("\nSample of positions:")
    #             for i, position in enumerate(positions_history['data'][:5], 1):
    #                 print(f"\nPosition {i}:")
    #                 print(f"  Instrument: {position.get('instId', 'N/A')}")
    #                 print(f"  Direction: {position.get('direction', 'N/A')}")
    #                 print(f"  Open Price: {position.get('openAvgPx', 'N/A')}")
    #                 print(f"  Close Price: {position.get('closeAvgPx', 'N/A')}")
    #                 print(f"  PnL: {position.get('pnl', 'N/A')}")
    #                 print(f"  Time: {position.get('cTime', 'N/A')}")
                
    #             if len(positions_history['data']) > 5:
    #                 print(f"\n... and {len(positions_history['data']) - 5} more positions")
    #         else:
...
# ------------------------------------------------------------------------
    ...
    #     print(self.AccountAPI.get_instruments(instType='SWAP'))
    ...
    def test_get_balance(self):
        print(self.AccountAPI.get_account_balance())
